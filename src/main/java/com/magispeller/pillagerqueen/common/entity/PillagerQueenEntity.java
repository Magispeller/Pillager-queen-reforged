package com.magispeller.pillagerqueen.common.entity;
...
import net.minecraft.world.damagesource.DamageTypes; // ADDED: 用于判断摔落伤害类型
...
public class PillagerQueenEntity extends Raider 
{
    ...
    public static AttributeSupplier.Builder createAttributes()
    {
        return Mob.createMobAttributes()
                ...
                .add(Attributes.ARMOR, 10.0D); // 新增：10点护甲
    }
	...
	public void handleAnimations()
	{
		...
		{
			if (onGround()) // CHANGED: isOnGround() -> onGround()
			{
		...
		if (!onGround() && !isNoGravity() && !isAggressive()) // CHANGED: isOnGround() -> onGround()
		{
	...
	public void handleEntityEvent(byte b)
	{
		...
		{
			for (int i = 0; i < 10; i++)
				this.level().addParticle(ParticleTypes.CRIT, getX(4.0D), getRandomY(), getZ(4.0D), 2.0D, 2.0D, 2.0D); // CHANGED: this.level -> this.level()
		} 
		else if (b == 98) 
		{
		...
		else if (b == 101) 
		{
			...
			{
				if (i % 2 == 0)
					level().addParticle(ParticleTypes.LARGE_SMOKE, getX() + Math.cos(i), getY(1.0D), getZ() + Math.sin(i), Math.cos(i), -0.25D, Math.sin(i)); // CHANGED: getLevel() -> level()
			}
			for (AbstractIllager p : this.level().getNearbyEntities(AbstractIllager.class, TargetingConditions.DEFAULT, (LivingEntity) this, getBoundingBox().inflate(25.0D))) // CHANGED: this.level -> this.level()
			{
				for (int j = 0; j < 32; j++)
					level().addParticle(ParticleTypes.POOF, p.getX(3.0D), p.getY(), p.getZ(3.0D), 0.25D, 0.25D, 0.25D); // CHANGED: getLevel() -> level()
			}
		}
		else 
		{
	...
	{
		// CHANGED: 统一使用新的伤害类型API判断是否为摔落伤害。同时将 this.level 替换为 this.level()
		if (!this.level().isClientSide && !hasSpawnedPatrol() && !damageSource.is(DamageTypes.FALL) && this.level().getGameRules().getBoolean(GameRules.RULE_DOMOBSPAWNING) && this.level().getGameRules().getBoolean(GameRules.RULE_DO_PATROL_SPAWNING))
		{
			ServerLevel serverLevel = (ServerLevel) this.level(); // CHANGED: this.level -> this.level()
			int targetY = level().getHeight(Heightmap.Types.WORLD_SURFACE, getBlockX(), getBlockZ()); // CHANGED: getLevel() -> level()
			BlockPos blockPos = BlockPos.containing(getX(10.0D), targetY, getZ(10.0D)); // CHANGED: new BlockPos(...) -> BlockPos.containing(...)
			setHasSpawnedPatrol(true);
			int amountOfAmbush = this.random.nextInt(8) + 1;
			for (int i = 0; i < amountOfAmbush; i++) 
			{
				blockPos = BlockPos.containing(getX(10.0D), targetY, getZ(10.0D)); // CHANGED: new BlockPos(...) -> BlockPos.containing(...)
				...
				level().playSound(null, patrollingMonster.getX(), patrollingMonster.getY(), patrollingMonster.getZ(), SoundEvents.PILLAGER_CELEBRATE, SoundSource.HOSTILE, 1.0F, 1.0F); // CHANGED: getLevel() -> level()
			}
			level().playSound(null, getX(), getY(), getZ(), ModSounds.PILLAGER_QUEEN_AMBUSH.get(), SoundSource.HOSTILE, 1.0F, 1.0F); // CHANGED: getLevel() -> level()
			level().broadcastEntityEvent(this, (byte) 101); // CHANGED: getLevel() -> level()
		}
		// CHANGED: 免疫摔落伤害的判断也改用新的API
	...
	class QueenMeleeAttackGoal extends Goal 
	{
		...
		{
			long l = this.mob.level().getGameTime(); // CHANGED: this.mob.level -> this.mob.level()
		...
		protected void checkAndPerformAttack(LivingEntity livingEntity, double d) 
		{
			...
			if (d <= e)
				if (this.damageTimer < 0 && getTicksUntilNextAttack() <= 0)
				{
					...
					this.mob.level().broadcastEntityEvent(this.mob, (byte) 98); // CHANGED: this.mob.getLevel() -> this.mob.level()
					this.damageTimer = 3;
				...
				else if (this.damageTimer == 0) 
				{
					...
					PillagerQueenEntity.this.level().playSound(null, PillagerQueenEntity.this.getX(), PillagerQueenEntity.this.getY(), PillagerQueenEntity.this.getZ(), ModSounds.PILLAGER_QUEEN_MELEE.get(), SoundSource.HOSTILE, 1.0F, 1.0F); // CHANGED: getLevel() -> level()
					this.mob.level().broadcastEntityEvent(this.mob, (byte) 97); // CHANGED: this.mob.getLevel() -> this.mob.level()
	...
	class QueenFlyingGoal extends Goal 
	{
		...
		public void start() 
		{
			...
			this.pillagerQueen.level().broadcastEntityEvent(this.pillagerQueen, (byte) 99); // CHANGED: getLevel() -> level()
		}
		...
		public void stop() {
			...
			this.pillagerQueen.level().broadcastEntityEvent(this.pillagerQueen, (byte) 100); // CHANGED: getLevel() -> level()
		}
		...
		public void tick()
		{
			...
			int targetY = this.pillagerQueen.level().getHeight(Heightmap.Types.WORLD_SURFACE, this.pillagerQueen.getBlockX(), this.pillagerQueen.getBlockZ()); // CHANGED: getLevel() -> level()
